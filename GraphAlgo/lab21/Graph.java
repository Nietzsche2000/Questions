...
public class Graph {
    ...
    public Graph prims(int start) {
        // TODO: YOUR CODE HERE
        Graph T = new Graph();
        HashMap<Integer, Integer> distToFromTree = new HashMap<>();
        HashSet<Integer> marked = new HashSet<>();
        HashMap<Integer, Edge> edgeTo = new HashMap<>();
        PriorityQueue<Integer> fringe = new PriorityQueue<>((x, y) -> Integer.compare(distToFromTree.get(x), distToFromTree.get(y)));
        distToFromTree.put(start, 0);
        fringe.add(start);
        while (!fringe.isEmpty()) {
            int vertex = fringe.poll();
            for (Edge e : this.getEdges(vertex)) {
                if (!marked.contains(e.getDest())) {
                    if (!distToFromTree.containsKey(e.getDest()) || distToFromTree.get(e.getDest()) > e.getWeight()) {
                        distToFromTree.put(e.getDest(), e.getWeight());
                        fringe.add(e.getDest());
                        edgeTo.put(e.getDest(), e);
                    }
                }
            }
            marked.add(vertex);
    ...
    public Graph kruskals() {
        // TODO: YOUR CODE HERE
        int totalVertexCount = this.getAllVertices().size();
        WeightedQuickUnionUF check = new WeightedQuickUnionUF(totalVertexCount);
        Graph T = new Graph();
        Iterator<Edge> i = this.getAllEdges().iterator();
        while (i.hasNext() && totalVertexCount > 1) {
            Edge e = i.next();
            int from = e.getSource();
            int to = e.getDest();
            if (check.connected(from, to))
                continue;
            check.union(from, to);
            T.addEdge(e);
            totalVertexCount--;
